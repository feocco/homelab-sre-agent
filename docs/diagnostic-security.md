...
The S3 handoff happens later, when an autofix is approved and the NAS-hosted SRE
agent dispatches the Codex workflow. At dispatch time, the SRE agent uploads a
bounded diagnostic summary to private S3 and creates a short-lived pre-signed
URL for that specific object. The URL is passed in the `repository_dispatch`
payload so the GitHub Actions workflow can fetch the diagnostic context before
running Codex.
...
homelab-sre-agent -> S3
  outbound HTTPS upload of diagnostic summary
...
- Diagnostic objects expire automatically.
...
after approval and without any route into the local network. For now, a
least-privilege long-lived IAM access key is acceptable. If this pattern expands,
move toward short-lived AWS credentials and stricter object-level audit.
